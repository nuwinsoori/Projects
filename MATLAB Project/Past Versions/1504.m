...
while money > 0 && quitGame == false
    ...
    while true; % player actions loop 
    ...
    % if player busts exits current hand and decreases money
    if bust == true 
        fprintf("You Lost!");
        money = money -bet;
        pause;
        continue; % leaves current hand
    end
    ...
    while true; %dealer hit loop 
        ...
        if dealerFinalValue >= 17 % breaks loop if dealer hits 17 or greater 
            clc;
    ...
    if finalValue == dealerFinalValue % checks if
        checkWinner = 2;
    end
...
% PRINTDEALERSHAND pritns the value of dealers cards 
