...
dealersHandImages = cell(6,1); % all dealers handstate images 
dealersCardValue = cell(6,1); % all dealers hand values  

% when player drawing 
dealersHandImages{1} = downCard + cardImage{2}; 

% give values for cards at different states 
dealersCardValue{1} = trueCardValue(dealersCardRanks{2}); % since first card is hidden to player  
dealersCardValue{2} = dealersCardValue{1} + trueCardValue(dealersCardRanks{1}); % when hidden card shown to player 

for i = 3:5
    dealersCardValue{i} = dealersCardValue{i-1} + trueCardValue(dealersCardRanks{i});
end


% when dealer drawing 
for i = 1:5
    dealersHandImages{i+1} = dealersJoinedBotCardImages{i} + dealersCardImages{i+1};
end
...
playersCardValue = cell(11,1);

playersCardValue{1} = trueCardValue(playersCardRanks{1}) + trueCardValue(playersCardRanks{2});

for i = 3:11
    playersCardValue{i-1} = playersCardValue{i-2} + trueCardValue(playersCardRanks{i});
end


% MAIN GAME 

money = 2; % starting money for player
...
fprintf(['By Nuwin Sooriyaarachchi (ID: a1900940)\n\n', ...
    ...
    '-If both you and the dealer have the same, it''s a tie and you get your bet back.\n\n']);

fprintf("Press enter to start game\n");
pause;
clc;

while money > 0 && quitGame == false; % checks player has enough money and wants to play game 
    ...
    handCounter = handCounter +1; % increases handCounter at end 
